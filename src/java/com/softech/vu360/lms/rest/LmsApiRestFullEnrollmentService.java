...
public class LmsApiRestFullEnrollmentService {
	...
	@ResponseBody
	public ResponseEntity<String> enrollUser(@RequestBody EnrollmentLearnerRestfulRequest request) throws Exception {
		...
		try {
			...
			if(customer == null){
				...
				return new ResponseEntity<String>(getjsonString(response), HttpStatus.BAD_REQUEST);
			}
		...
		return new ResponseEntity(getjsonString(response), requestStatus);
